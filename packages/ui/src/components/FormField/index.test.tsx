...
describe('[Component] FormField', () => {
  ...
  describe('클리어 버튼:', () => {
    ...
    test('클리어 버튼 클릭 시 autoFocusAfterClick 속성이 true일 때 폼 요소가 포커스된다.', async () => {
      const { user, getByRole } = setup({
        clearProps: { visible: true, autoFocusAfterClick: true },
      })
    ...
    test('클리어 버튼 클릭 시 autoFocusAfterClick 속성이 false일 때 폼 요소가 포커스되지 않는다.', async () => {
      const { user, getByRole } = setup({
        clearProps: { visible: true, autoFocusAfterClick: false },
      })

      await user.click(getByRole('button', { name: 'Clear' }))

      expect(getByRole('textbox', { name: 'First Name' })).not.toHaveFocus()
    })
  })
})
